...
piInput :: (MonadWidget t m) => m (Dynamic t String)
piInput = do
  clickHistory <- keypadAndKeyboard
  mirroredInput <- foldDyn reducer "" clickHistory
  mapDyn reverse mirroredInput
...
keypadWithDelete :: MonadWidget t m => m (Event t KeypadEvent)
keypadWithDelete = do
  k <- keypad
